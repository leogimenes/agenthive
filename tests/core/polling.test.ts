...
describe('AgentLoop', () => {
  ...
  describe('cycle() — worktree sync', () => {
    ...
    it('should still check requests even when idle sync fails', async () => {
      ...
      // Chat requests are checked BEFORE idle sync (chat always accessible)
      expect(vi.mocked(findRequests)).toHaveBeenCalled();
    });
  });
